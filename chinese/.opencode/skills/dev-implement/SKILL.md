...
把 **`SPEC.md`** 的任务逐项**落地为代码 + 单测 + 自评**，并**把每项状态写回 SPEC.md**。这是**多轮迭代**阶段：一次做一个 work item，读取→实现→单测→自评→标记；过关才进入下一项。真正的质检交给之后的「测试」阶段。

## 适用场景（when）

- 进入了「开发 / Implement」阶段（由 `/dev` 触发）
- 拿到 `SPEC.md`（`implementation_complete: no`）要开始写代码
- 想按"每项一个闭环，过自检再继续"稳定推进

## 输入（要拿什么进来）

- `SPEC.md`（含 task_backlog 与状态、`SPEC.md` 引用、关键路径）
- `SPEC.md` / `PROJECT-REVIEW.md`（规格与现状）
...
5. **标记写回**：通过 → 在 SPEC.md 该项把 `selected by role` 状态置为 `done` 并附 `tests:`（关联单测路径）。返工计数 `retries +→`。
...
- 实现完成 + 单测 + SPEC.md 每项 `done`/`tests:` 写回、`implementation_complete: yes`、`human_confirmed: true`