...
- **`GATE_REQUIREMENTS.md`**（工程根，定义"需求门禁规范"；缺失则先请用户补充）

## 核心步骤（流程，严格按序）

1. **确认输入就绪**：`GATE_REQUIREMENTS.md` 存在；`PROJECT-REVIEW.md` 存在。缺失 → 先补齐。
2. **收集需求**：用 `question` 工具收集"要设计什么"。不套死格式；不同项目形态不同。
3. **规范机控**：把需求逐项对照 `GATE_REQUIREMENTS.md` 检核。不达标 → 打回澄清，达标才继续。
...
- [ ] `GATE_REQUIREMENTS.md` 存在，且需求已按其逐项检核通过？
...
- ❌ 没有需求就设计 → 无的放矢；无 `GATE_REQUIREMENTS.md` 也硬做
- ❌ 需求不 sign-off 就往下 → 返工
