...
1. **读输入**：`PLAN.md`（实现清单）、`DESIGN.md`（验收口径）、`GATE-REQUIREMENTS.md`（性能/安全阈值）与被测代码。
...
5. **缺陷管理**：发现缺陷 → `bug-report-writing` 落地缺陷单并**写入 `issues.md`**（回流 `/dev` 或 `/design`）。
6. **测试评审**：`@quality-assurance` 复核覆盖与结论。
7. **产出 `TEST.md`**：frontmatter `result: pass|fail`、`defects`、`defects_flowed`。
8. **人工确认**：用 `question` 汇报结论与缺陷；确认后置 `human_confirmed: true`（`fail` 则按回流去 `/dev`/`/design` 修）。
