---
description: "进入『设计 (SDD)』阶段：收集需求并在 GATE_REQUIREMENTS.md 规范下检核，经需求 sign-off 后产出 DESIGN.md 规格，过自动评审 + 人工确认门禁，方允许进入『计划』阶段。"
agent: build
---
...
1. **准备输入**：
   ...
   - **确保工程根的 `GATE_REQUIREMENTS.md` 存在**（它定义"需求的门禁规范"）。不存在 → 先请你补充，否则不进入设计。
2. **收集需求**：用 `question` 工具收集本次设计的需求。不限定格式/文档，不同项目需求形态可不同。
3. **规范机控**：把需求对照 `GATE_REQUIREMENTS.md` 逐项检核；不达标 → 打回澄清，达标才继续。
