---
description: "进入『开发 (Dev/Implement)』阶段：按 dev-implement 技能逐项执行 PLAN.md 的 work items（实现+单测+自评），把状态写回 PLAN.md，过人工确认后交付给『测试』阶段。"
agent: build
---

你是本阶段的执行者（build 徽章），执行「开发 (Dev/Implement)」。加载 `dev-implement` 技能并按其流程执行。逐项把 **`PLAN.md`** 的任务落地为代码 + 单测 + 自评，并把状态写回 PLAN，完成后才允许进入「测试」阶段（`dev-phase-gate` 会硬校验）。

## 前置
- `PLAN.md` 已 `status: complete`（`plan-phase-gate` 已放行 `/dev`）。
